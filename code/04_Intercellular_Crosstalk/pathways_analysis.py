...
with app.setup:
    ...
    go_snapshot = data_dir / "processed" / "go_biological_process_2025.json"
    required_inputs = [
        data_dir / "archetype_lr_pairs.csv",
        data_dir / "archetypes_tissue_correlation.csv",
        data_dir / "processed" / "ligand_receptor_lists.json",
        data_dir / "raw" / "1GO_term_summary_20220615_085602.xlsx",
        go_snapshot,
    ]
    ...
    with go_snapshot.open(encoding="utf-8") as _f:
        go_bp = json.load(_f)
...
@app.cell
def _():
    lr_pairs = pd.read_csv(data_dir / "archetype_lr_pairs.csv")
    # lr_pairs
    return (lr_pairs,)
...
def get_lr_from_go(
    ...
    with open(local_data_dir / "processed" / "ligand_receptor_lists.json") as _f:
    ...
    def map_from_key_term(key_term):
        ...
        for _go in key_term_go_list:
            try:
                ...
                raise RuntimeError(f"Failed to render GO process {_go!r}") from e
        mo.output.append(
            interactions(lr_pairs)
            .tissue_correction(tissues)
...
def _(lr_pairs, tissues):
    ...
    reg_of_angiogenesis.df.to_csv(results_dir / "fig_d_data.csv")
    reg_of_angiogenesis_plt = reg_of_angiogenesis.condense().plot(title="Regulation of Angiogenesis (GO:0045765)", lw_scaler=1)
    reg_of_angiogenesis_plt.savefig(results_dir / "fig_d_angiogenesis.svg", format='svg')
...
def _(lr_pairs, tissues):
    ...
    ecm_organization.df.to_csv(results_dir / "fig_f_data.csv")
    ecm_organization_plt = ecm_organization.condense().plot(title="Extracellular Matrix Organization (GO:0030198)", lw_scaler=1)
    ecm_organization_plt.savefig(results_dir / "fig_f_ecm.svg", format='svg')
...
def _(lr_pairs, tissues):
    ...
    reg_inflamitory.df.to_csv(results_dir / "fig_e_data.csv")
    reg_inflamitory_plt = reg_inflamitory.condense().plot(title="Regulation of Inflammatory Response (GO:0050727)", lw_scalar=1)
    reg_inflamitory_plt.savefig(results_dir / "fig_e_inflamation.svg", format="svg")
    reg_inflamitory_plt
    return
...
@app.cell
def _():
    mo.md(r"""## Growth factor""")
    return


@app.cell
def _():
    growth_factor_go_list = list(set(pd.read_excel(local_data_dir / "raw" / "1GO_term_summary_20220615_085602.xlsx")['Symbol']))
...
def _(growth_factor_go_list, lr_pairs, tissues):
    interactions(lr_pairs).tissue_correction(tissues).min_mean_diff(0.4).slice_by_gene_list(get_lr_from_gene_list(growth_factor_go_list)).condense().plot(title="growth factor activity (GO:0008083)")
    return


@app.cell
def _(growth_factor_go_list, lr_pairs, tissues):
    growth_factors = interactions(lr_pairs).tissue_correction(tissues).min_mean_diff(0.4).slice_by_gene_list(get_lr_from_gene_list(growth_factor_go_list))
    growth_factors.df.to_csv(results_dir / "fig_g_data.csv")
    growth_factors_plt = growth_factors.condense().plot(title="growth factor activity (GO:0008083)", lw_scalar=1)
    growth_factors_plt.savefig(results_dir / "fig_g_growth_factors.svg", format="svg")
    growth_factors_plt
    return
